...
title(sprintf('Total Azimuth Diversity (%s)', TRAJECTORY_TYPE));
...
polarplot(bearings(end)*pi/180, distances_from_emitter(end), 'ro', ...
          'MarkerSize', 12, 'LineWidth', 2);
title('Bearing Diversity (Polar View)');;
...
title('Interferometer Measurements (X-Z Plane Angle)');;
...
title('True Elevation Angle (NED frame)');;
...
ylabel('Y (right)');
zlabel('Z (down)');
title('Measurement Geometry in Body Frame');
axis equal;
view(-45, 20);
xlim([-1.2, 1.2]);
ylim([-1.2, 1.2]);
...
% Add overall title
sgtitle(sprintf('%s Trajectory: Body Frame Geometry Snapshots', upper(TRAJECTORY_TYPE)), ...
...
for plot_idx = 1:n_verify_samples
    ...
    grid on;
    ylabel('Distance (km)');
    title(sprintf('Body Frame Components\nAngle: %.2f° (Meas: %.2f°)', ...
...
if RUN_PARAMETER_SWEEP && exist('results', 'var')
    ...
    title(sprintf('%s: Q vs R Impact', upper(TRAJECTORY_TYPE)));
    colorbar;
    grid on;
    view(-45, 30);
    ...
    ylabel('Count');
    title(sprintf('Error Distribution (%s)', TRAJECTORY_TYPE));
    legend('Location', 'best');
    grid on;
