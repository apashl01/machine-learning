...
def monte_carlo_analysis(receiver: ReceiverParameters,
                         radar: RadarParameters,
                         n_iterations: int = 10000,
                         param_bounds: Optional[Dict] = None,
                         random_seed: Optional[int] = None) -> Dict:
    """
    Monte Carlo analysis of ESM detection performance.

    Args:
        receiver: Base ReceiverParameters
        radar: Base RadarParameters
        n_iterations: Number of random samples
        param_bounds: Dict of parameter bounds, e.g.,
            {'eirp_dBW': (30, 100), 'noise_figure_dB': (3, 25)}
    ...
    results = {
        'detection_range_km': np.zeros(n_iterations),
        'samples': {k: np.zeros(n_iterations) for k in param_bounds.keys()},
    }
    ...
    for i in range(n_iterations):
        ...
        for param, (low, high) in param_bounds.items():
            ...
            results['samples'][param][i] = val

            if hasattr(radar_copy, param):
                setattr(radar_copy, param, val)
            elif hasattr(rx_copy, param):
                setattr(rx_copy, param, val)

        detector = ESMDetector(rx_copy)
        results['detection_range_km'][i] = detector.calculate_detection_range(radar_copy)

    # Calculate statistics
    results['statistics'] = {
        'mean': np.mean(results['detection_range_km']),
        'median': np.median(results['detection_range_km']),
        'std': np.std(results['detection_range_km']),
        'p5': np.percentile(results['detection_range_km'], 5),
        'p95': np.percentile(results['detection_range_km'], 95),
        'min': np.min(results['detection_range_km']),
        'max': np.max(results['detection_range_km']),
    }

    # Calculate correlations
    results['correlations'] = {}
    for param in param_bounds.keys():
        corr = np.corrcoef(results['samples'][param],
                           results['detection_range_km'])[0, 1]
        results['correlations'][param] = corr
